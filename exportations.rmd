...
bivariate_plot(m,v,"Mean", "Variance")
```


```{r}
boxplot_ser(ser, "Year", "Bilions of euros")
```
...
plot_ser(d12trser, ylab = "", red_line_at_0 = TRUE)
```

```{r}
monthplot_ser(d12trser)
```


```{r}
seasonal_ser(d12trser)
```
...
plot_ser(d1d12trser, ylab = "", red_line_at_0 = TRUE)
```
...
bivariate_plot(1:length(resi), sqrt(abs(resi)), "", "")
```
...
Un $p-valor = 0.04701$ suggereix que no hi ha evidència suficient com per afirmar que els residus estiguin normalment distribuits.
...
Un $p-valor=0.006186$ és molt baix, el que ens indica que no hi ha prou evidència als residus com per afirmar que son normals.
...
plot_predicted(ser, pr, tl, tu, 2013, 2019)
```
...
plot_forecast(ser, pr, tl, tu, 2016, 2020)
```
...
plot_predicted(ser, pr, tl, tu, 2016, 2019)
```
...
plot_ser_lin(ser, (lambda * trser.lin + 1)^(1 / lambda), "Billions of euros")
```

```{r}
plot_ser(trser - trser.lin, "outlier effect")
```
...
```{r}
plot_acf_pacf(d1d12trserEC.lin, 72)
```
